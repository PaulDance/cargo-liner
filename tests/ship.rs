...
#[cargo_test]
fn validate_ship_install_target_cli() {
    let _reg = init_registry();
    fake_install_self();
    fake_publish("abc", "0.0.0");
    write_user_config(&["[packages]", "abc = '*'"]);

    // HACK: make it fail to display the successfully-passed argument.
    cargo_liner!()
        .args([
            "ship",
            "--no-self",
            "--target",
            "noarch-novendor-nonexistingsys-noabi",
        ])
        .assert()
        .failure()
        .stdout_eq("".into_data().raw())
        .stderr_eq(snapbox::file![
            "fixtures/ship/validate_ship_install_target.stderr"
        ]);
    assert_not_installed("abc");
}

#[cargo_test]
fn validate_ship_install_target_config() {
    let _reg = init_registry();
    fake_install_self();
    fake_publish("abc", "0.0.0");
    write_user_config(&[
        "[packages]",
        "abc = { version = '*', target = 'noarch-novendor-nonexistingsys-noabi' }",
    ]);
...
/// Tests `bins` for the Binstall method.
#[cargo_test(requires = "cargo-binstall")]
fn validate_ship_binstall_bins() {
    let _reg = init_registry();
    fake_install_self();
    fake_publish("abc", "0.0.0");
    write_user_config(&[
        ...
        "ship.binstall = 'always'",
        "[packages]",
        "abc = { version = '*', registry = 'dummy-registry', bins = ['yo', 'watup'] }",
    ]);
    ...
    cargo_liner!()
        ...
        .stdout_eq("".into_data().raw())
        .stderr_eq(snapbox::file![
            "fixtures/ship/validate_ship_binstall_bins.stderr"
        ]);
    assert_not_installed("abc");
}
